...
namespace AutoMLGUI.RunModules
{
    internal class RunGridSearch
    {
        ...
        public static bool ValidateForm(Dictionary<string, Control> controlMap)
        {
            try
            {
                // Define required integer fields (TextBoxes)
                var integerFields = new Dictionary<string, string>
        {
            ...
            { "GRIDSEARCHCV_N_JOBS", "gridSearchNJobsTextBox" }
                ...
                // Define required integer fields (ComboBoxes)
                ...
                var stringFields = new Dictionary<string, string>
        {
            { "SCORING_METRIC", "gridSearchScoringMetricComboBox" }
                ...
                // Define required boolean fields (checkbox validation)
                var booleanFields = new Dictionary<string, string>
        {
            { "REFIT_BEST_MODEL", "gridSearchReFitBestModelCheckBox" }
        };
                ...
                // Validate boolean checkbox fields
                foreach (var field in booleanFields)
                {
                    if (!controlMap.TryGetValue(field.Value, out Control control) || !(control is CheckBox checkBox))
                    {
                        MessageBox.Show($"❌ Validation failed: {field.Key} checkbox is missing!", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
                }
