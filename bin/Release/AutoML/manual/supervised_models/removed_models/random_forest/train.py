...
def train_and_save_model(logger):
    try:
        ...
        n_estimators = getattr(config, 'RANDOM_FOREST_N_ESTIMATORS', 100)
        max_depth = getattr(config, 'RANDOM_FOREST_MAX_DEPTH', None)
        min_samples_split = getattr(config, 'RANDOM_FOREST_MIN_SAMPLES_SPLIT', 2)
        min_samples_leaf = getattr(config, 'RANDOM_FOREST_MIN_SAMPLES_LEAF', 1)
        criterion = getattr(config, 'RANDOM_FOREST_CRITERION', 'gini')

        # Validate hyperparameters
        if not isinstance(n_estimators, int) or n_estimators <= 0:
            logger.warning("Invalid n_estimators. Using default value: 100.")
            n_estimators = 100
