...
def predict_from_csv(data, processed_data, logger):
    """
    Predict from a CSV file using a pre-trained CatBoost model and save results with confidence scores.
    """
    try:
        ...
        if hasattr(model, 'feature_names_'):
            training_features = model.feature_names_
        ...
        column_mapping = {col.strip().lower().replace(" ", "_"): col for col in data.columns}
        ...
        predictions = le.inverse_transform(raw_predictions.astype(int))  # Convert raw predictions to integer before inverse transform
