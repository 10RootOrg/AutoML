...
def train_and_save_model(logger):
    try:
        ...
        learning_rate = getattr(config, 'CATBOOST_LEARNING_RATE', 0.03)
        depth = getattr(config, 'CATBOOST_DEPTH', 6)

        # Validate hyperparameters
        if not isinstance(iterations, int) or iterations <= 0:
            logger.warning("Invalid iterations. Using default value: 1000.")
            iterations = 1000
        if not isinstance(learning_rate, (float, int)) or learning_rate <= 0:
            logger.warning("Invalid learning_rate. Using default value: 0.03.")
            learning_rate = 0.03
        if not isinstance(depth, int) or depth <= 0:
            logger.warning("Invalid depth. Using default value: 6.")
            depth = 6
        ...
        if hasattr(model, "feature_importances_"):
            ...
            sns.barplot(x=feature_importances['Importance'], y=feature_importances['Feature'], color='skyblue')
            ...
            logger.info(f"Feature importances plot saved to {plot_path}")
            plt.close()
